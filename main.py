...
def readFile(path):
    ...
    for i in range(0,n_requests):
        ...
        if id_video not in endpoints[endpoint_id_request].requests:
            endpoints[endpoint_id_request].requests[id_video] = 0
        ...
        for video, Nrequests in endpoint.requests.items():
            ...
            if len(cache_rank_list):
                ...
                for c in cache_rank_list:
                    caches[c].remove_rank_from_endpoint(video, cache_rank_values[c])
