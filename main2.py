...
class Cache:
    ...
    def add_rank_from_endpoint(self,video, points):
        self.rank_values[video] += points

    def remove_rank_from_endpoint(self,video, points):
        self.rank_values[video] -= points

    def get_position(self,video):
        if video not in self.rank_list:
            print("Error")
        return self.rank_list.index(video) +1
    ...
    for endpoint in endpoints.values():
        ...
        for video, Nrequests in endpoint.requests.items():
            ...
            for cache_id, cache_latency in endpoint.cache_latency.items():
                ...
                cache_rank_values[cache_id] =  cache_latency

            cache_rank_list = sorted(cache_rank_values, key=cache_rank_values.__getitem__)
            if len(cache_rank_list):
                best_cache_id = cache_rank_list[0]
                caches[best_cache_id].add_rank_from_endpoint(video,1)
                for c in cache_rank_list:
                    caches[c].remove_rank_from_endpoint(video, 1)
